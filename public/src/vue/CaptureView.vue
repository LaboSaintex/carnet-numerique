<template>
        ...
        <div class="m_panel--buttons">
          ...
          <transition name="slideup" :duration="400">
            <MediaValidationButtons
              ...
              @cancel="cancelValidationAndReset()"
              @save="sendMedia({})"
            />
          </transition>
        </div>
  ...
  <div class="m_captureview--modeSelector">
    ...
    <DistantFlux
      v-if="$root.settings.capture_options.distant_flux.active"
      :key="
        ($root.settings.capture_options.distant_flux.username = current_username)
      "
      @changeStreamTo="
        (new_stream) => {
  ...
  watch: {
    saveVideo: function() {
      if(this.saveVideo) {
        ...
        console.log("DEMANDING SAVING VIDEO IN PROJECT DIRECTORY");
      }
    },
