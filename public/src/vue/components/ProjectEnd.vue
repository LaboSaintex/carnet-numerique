...
export default {
  ...
  methods: {
    generateOrReadVideo: function() {
      ...
      this.updatePublicationsFolder();
      this.$root.openPublication(this.$root.currentProject.slugFolderName.toLowerCase());
      let publication = this.$root.store.publications[this.$root.currentProject.slugFolderName.toLowerCase()];
      this.$root.current_publication = publication;
    ...
    updateMediasPubli() {
      ...
      const currentProjectMedias = Object.keys(this.$root.currentProject.medias)
      ...
      currentProjectMedias.map(item => {
        ...
        if(!this.$root.current_publication.medias.hasOwnProperty(metaFileName)) {
  ...
  computed: {
    alreadyHasPublicationVideo: function() {
      return Object.keys(this.$root.store.publications).
        indexOf(this.$root.currentProject.slugFolderName.toLowerCase()) != -1;
    }
  },
  created() {
